...
if(isset($_GET["s"])){
    $search = $_GET["s"];
    $query = "select * from users where role = 'Fournisseur' and (nom like '%" . $search . "%' or prenom like '%" . $search . "%' or email like '%" . $search . "%' or telephone like '%" . $search . "%') order by ref_client desc";
}
else {
