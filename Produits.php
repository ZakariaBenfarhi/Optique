...
//if(!empty($_SESSION["role"]) && $_SESSION["role"] == "Opticien"){
    ...
    if(isset($_GET["s"])){
        $search = $_GET["s"];
        $query = "select * from produit p, categorie c where c.ref_cat = p.ref_cat and (c.descrip_cat like '%" . $search . "%' or p.descrip like '%" . $search . "%' or p.pour like '%" . $search . "%' or p.ref_lentille like '%" . $search . "%' or p.type_lentille like '%" . $search . "%') order by p.ref_Produit desc";
    }
    else {
    			...
    			<?php if(mysqli_num_rows($rs) != 0){ ?>
    				<table class="table table-hover table-sm" id="sortTable">
    					...
    					<tr>
    						...
    						<td style="text-align: center;"><?php echo $et["largeur"] ?></td>
